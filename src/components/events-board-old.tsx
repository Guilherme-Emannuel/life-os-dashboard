...
import { Trash2, Plus, Calendar, Flag } from "lucide-react";

// Definição manual de tipos para evitar o erro de build do Prisma no Browser
type Priority = "LOW" | "MEDIUM" | "HIGH" | "CRITICAL";
type EventStatus = "PENDING" | "IN_PROGRESS" | "COMPLETED" | "OVERDUE";
type EventType = "TASK" | "EVENT" | "REMINDER";

// Interface manual para o Evento (substituindo o tipo do Prisma)
...
export function EventsBoard({ moduleId }: { moduleId?: string }) {
    ...
    mutationFn: async () => {
      ...
      const res = await fetch("/api/events", {
        ...
        body: JSON.stringify({
          title,
          type,
          status: "PENDING", // String pura em vez de EventStatus.PENDING
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="grid gap-2 md:grid-cols-[2fr,1fr]">
          <div className="space-y-2 text-xs">
            <input
              ...
              placeholder="Título rápido (ex.: pagar boleto...)"
            ...
            <textarea
              ...
              placeholder="Briefing inicial (opcional)..."
              rows={2}
          ...
          <div className="flex flex-col gap-2 text-xs">
            ...
            <button
              ...
              disabled={createMutation.isPending || !title || !deadline || !moduleId}
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4">
        <p className="mb-3 text-xs font-medium text-zinc-300">Lista de eventos</p>
              ...
              >
                <summary className="flex cursor-pointer list-none items-center justify-between gap-2 text-xs">
                  <div className="flex items-center gap-2">
                    <span className={`h-1.5 w-6 rounded-full ${priorityColorMap[event.priority] || "bg-zinc-500"}`} />
                    <span className="truncate">{event.title}</span>
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-zinc-400">
                    <span>{new Date(event.dueDate).toLocaleString("pt-BR")}</span>
                    <button
                      type="button"
                      onClick={(e) => {
                        e.preventDefault();
                ...
                <div className="mt-2 space-y-1 text-[11px] text-zinc-300">
                  {event.brief && <p className="text-zinc-300">{event.brief}</p>}
                  {event.attachments.length > 0 && (
                    <div className="mt-1 flex flex-wrap gap-1.5">
                      {event.attachments.map((a) => (
                        <a key={a.id} href={a.url} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-full border border-zinc-700 bg-zinc-900 px-2 py-0.5 text-[10px] text-zinc-200 hover:border-emerald-400">
                          Anexo
                        </a>
                      ))}
                    </div>
